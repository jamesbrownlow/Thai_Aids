...
```{r}
AidsResults <- read.csv(file = "ThaiAidsTrial.csv")
trialtbl=xtabs(~results+treatment,data=AidsResults)
```
...
```{r}
dd2 <- AidsResults %>% group_by(results,treatment) %>% summarize(count=n()) %>% mutate(prcnt=count/sum(count))

basicC <- ggplot(dd2,aes(x=results,y=count,fill=treatment))
basicC + geom_bar(stat="identity",position="dodge")
#Now for percentage plot

basicCC <- ggplot(dd2,aes(x=results,y=prcnt*100,fill=treatment)) 
